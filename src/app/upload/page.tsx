    ...
    return (
        <div className="flex flex-col h-screen w-full bg-[#FAFAFA] text-slate-900 font-sans">
            ...
            <main className="flex-1 flex flex-col items-center justify-center p-6">
                <div className="w-full max-w-3xl bg-white p-10 rounded-3xl border border-slate-200 shadow-sm">
                    ...
                    {!previewUrl ? (
                        <div
                            className={`relative border-2 border-dashed rounded-2xl p-12 transition-all duration-200 flex flex-col items-center justify-center text-center cursor-pointer min-h-[360px]
                ${isDragging ? 'border-blue-500 bg-blue-50' : 'border-slate-300 bg-slate-50 hover:bg-slate-100 hover:border-slate-400'}`}
                        ...
                        <div className="flex flex-col items-center">
                            <div className="relative w-full aspect-video bg-slate-100 rounded-2xl border border-slate-200 overflow-hidden shadow-inner flex items-center justify-center mb-6">
                                <img
                                    src={previewUrl}
                                    alt="도면 미리보기"
                                ...
                                <button
                                    ...
                                    className="absolute top-4 right-4 w-8 h-8 bg-white/90 backdrop-blur text-slate-600 hover:text-red-500 rounded-full flex items-center justify-center shadow-sm transition-colors"
                                >
                            ...
                            {/* 업로드된 파일 정보 */}
                            <div className="flex items-center gap-3 w-full bg-slate-50 border border-slate-200 rounded-xl p-4">
                                <div className="w-10 h-10 bg-white border border-slate-200 rounded-lg flex items-center justify-center text-blue-600 shrink-0">
                                    <ImageIcon size={20} />
                                </div>
                                <div className="flex-1 min-w-0">
                                    <p className="text-[14px] font-bold text-slate-900 truncate">{selectedFile?.name}</p>
                                    <p className="text-[12px] text-slate-500">{(selectedFile?.size ? selectedFile.size / (1024 * 1024) : 0).toFixed(2)} MB</p>
                                </div>
                                <Link href="/editor" className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-lg font-semibold flex items-center gap-2 transition-colors text-sm">
